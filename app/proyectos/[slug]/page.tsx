        ...
        <div className="py-8 lg:py-20 flex flex-col gap-y-4 relative max-w-xl px-4 lg:px-0 opacity-0 animate-fade-left">
          <header className="flex flex-col lg:flex-row lg:justify-between lg:items-end gap-y-2">
            <div>
              ...
              <h1 className="text-4xl lg:text-6xl font-bold">{data.title}</h1>
            </div>
          ...
          <p className="leading-tight text-sm whitespace-break-spaces font-medium ">
            {data.text}
          </p>
          ...
          <div className="flex items-center justify-between mt-4">
            {data.advance !== 0 && (
              <div className="flex flex-col text-xl lg:text-2xl leading-tight">
                <div>
                  <span className="font-bold">{data.advance}%</span> anticipo
                </div>
                <div>
                  <span className="font-bold">+{data.pays}</span> cuotas
                </div>
              </div>
            )}
            <div>
              ...
              >
                <span className="font-bold lg:text-lg">Consultanos</span>
                <span className="bg-white text-secondary h-8 w-8 rounded-full flex items-center justify-center">
                  <WhatsApp />
                </span>
              </a>
